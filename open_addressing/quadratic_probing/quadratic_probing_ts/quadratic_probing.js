console.log("----- Results -----\n-------------------\n");
var SomeHashTable = /** @class */ (function () {
    function SomeHashTable() {
        this.table = [];
        this.mod = 41;
        this.numItems = 0;
        this.min = 29;
    }
    SomeHashTable.prototype.modularHash = function (key, attempt) {
        var sum = 0;
        for (var i = 0; i < key.length; ++i) {
            sum += key.charCodeAt(i);
        }
        ;
        var hash = (sum + Math.pow(attempt, 2)) % this.mod;
        if (isNaN(hash)) {
    ...
    SomeHashTable.prototype.resize = function (direction) {
        console.log("risizing ".concat(direction == 1 ? "up." : "down."));
        ...
        for (var i = 0; i < oldTable.length; i++) {
            if (oldTable[i] === undefined) {
                ...
                console.log("exists");
                var hash = this.modularHash(oldTable[i][0], 0);
                var attemptNum = 1;
    ...
    SomeHashTable.prototype.put = function (key, value) {
        var attemptNum = 0;
        this.numItems++;
    ...
    SomeHashTable.prototype.get = function (key) {
    ...
    SomeHashTable.prototype.remove = function (key) {
        for (var i = 0; i < this.mod + 1; ++i) {
            if (this.table[this.modularHash(key, i)] === undefined) {
                continue;
            }
...
console.log("\nPut phase\n----------\n");
var someHashTable = new SomeHashTable();
...
// console.log(someHashTable);
// console.log("\nRemove phase\n -------------\n");
var getResult = someHashTable.get("Raphael Cabrera");
